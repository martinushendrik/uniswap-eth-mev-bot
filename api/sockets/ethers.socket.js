            ...
            if (activeStatus) {
                ...
                if (fromToken.toLowerCase() === plan.fromToken.toLowerCase()) {
                    ...
                    tx = await uniswapContract.swapETHForExactTokens(
                        ...
                        [fromToken, toToken],
                        wallet_public,
                        currentTimestamp + 60 * 1000, // set deadline to 1000 minutes from now
                        gasTx
                    );
        ...
        try {
            ...
            if (activeStatus) {
                ...
                if (fromToken.toLowerCase() === plan.fromToken.toLowerCase()) {
                    ...
                    tx = await uniswapContract.swapExactTokensForTokensSupportingFeeOnTransferTokens(//swapExactTokensForETH
                        tokenAmountTosell,
                        amountOutMin,
                        [fromToken, toToken],
                        wallet_public,
            ...
            wsProvider.on("pending", async (txHash) => {
                ...
                try {
                    ...
                    if (tx.to && tx.to.toLowerCase() === uniswap.toLowerCase() && tx.from.toLowerCase() !== plan.public.toLowerCase() && tx.from !== "0x0000000000000000000000000000000000000000") {
                        ...
                        console.log("~~~~~~~~ Init Mempool ---first If ~~~~~~~~", functionName);
